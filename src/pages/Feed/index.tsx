...
const Feed: React.FC = () => {
  ...
  const favoritedPiusIdsCallback = useCallback(
    (pius: Array<PiuData>) => {
      const favoritedPius = pius.filter((piu) => {
        const usuariosQueFavoritaram = piu.favoritado_por.map(
          (item: UserApi) => item.id
        );
  ...
  const favoritedPiusIds = useMemo(() => favoritedPiusIdsCallback(pius), [
    user,
    pius,
  ]);
  ...
  const setSortedPius = useCallback(
    (newPius: Array<PiuData>) => {
      const favoritedPiusIdsLocal = favoritedPiusIdsCallback(newPius);

      function compareFavorite(a: PiuData, b: PiuData) {
        return (
          (favoritedPiusIdsLocal.includes(b.id) ? 1 : 0) -
          (favoritedPiusIdsLocal.includes(a.id) ? 1 : 0)
        );
      }
  ...
  const sendPiu = useCallback(
    async (mensagemInput: string) => {
      const response = await axios({
        url: "http://piupiuwer.polijr.com.br/pius/",
        method: "POST",
        headers: {
